...
Usage:
    $ cd <カレントディレクトリ>
    $ poetry run python ./moa-dnn-feature-engineering_20201028.py
"""
import sys
...
# モデルファイル出力先
OUT_MODEL = "model_20201028"
os.makedirs(OUT_MODEL, exist_ok=True)

# log lossの結果書き込むファイル
OOF_TXT = f"{OUT_MODEL}/oof_metrics.txt"
...
def run_submission(model_dir="../input"):
    ...
    # Blend 5l, 4l, 3l and l2 dnn model
    oof_pred = np.average([oof_pred_5l, oof_pred_4l, oof_pred_3l, oof_pred_2l], axis=0)
    seed_log_loss1 = mean_log_loss(train_targets.values, oof_pred)
    print(
        f"Our final out of folds log loss for our classic dnn blend is {seed_log_loss1}"
    )
    ...
    # Blend the result of the previous model with the dnn resnet type model
    oof_pred = np.average([oof_pred, oof_pred_rs], axis=0)
    seed_log_loss2 = mean_log_loss(train_targets.values, oof_pred)
    print(
        f"Our final out of folds log loss for our classic dnn + dnn resnet type model is {seed_log_loss2}"
    )
    test_pred = np.average([test_pred, test_pred_rs], axis=0)
