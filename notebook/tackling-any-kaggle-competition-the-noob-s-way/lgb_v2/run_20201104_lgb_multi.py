...
- drug_MultilabelStratifiedKFold(n_splits=5, shuffle=False)
- データ全体を使用
- oof で計算
- feature engineering
- nonscored + ClassifierChain
- params_py ディレクトリからパラメータ取得
- その他コード修正
...
Usage:
    ...
    $ python run_20201104_lgb_multi.py -p chain_non | Tee-Object -FilePath ./run_20201104_lgb_multi/train.log
    $ python run_20201104_lgb_multi.py -m objective -p chain_non | Tee-Object -FilePath ./run_20201104_lgb_multi/objective.log

    $ poetry run python run_20201104_lgb_multi.py -p chain -is_o_non 2>&1

    $ python run_20201104_lgb_multi.py -p multi # MultiOutputClassifierモデル作成
    $ python run_20201104_lgb_multi.py -p chain  # ClassifierChainモデル作成
    $ python run_20201104_lgb_multi.py -p chain_non  # nonscored + ClassifierChainモデル作成
    $ python run_20201104_lgb_multi.py -d -p multi # デバッグ
    $ python run_20201104_lgb_multi.py -m objective -p multi # パラメータチューニング
"""
import os
import sys
import glob
...
parser.add_argument(
    "-p", "--PRAMS_NAME", type=str, help="params.py name.",
)
...
OUTDIR = r"run_20201104_lgb_multi"
...
# パラメータファイルロード
if args["PRAMS_NAME"] == "multi":
    from params_py import params_lgb_multi as p_py

    IS_CHAIN = False  # ClassifierChain flag
    IS_ORDER_NONSCORED = False
    OUTDIR = f"{OUTDIR}/multi"

elif args["PRAMS_NAME"] == "chain":
    from params_py import params_lgb_chain as p_py

    IS_CHAIN = True  # ClassifierChain flag
    IS_ORDER_NONSCORED = False
    OUTDIR = f"{OUTDIR}/chain"

elif args["PRAMS_NAME"] == "chain_non":
    from params_py import params_lgb_chain_nonscored as p_py

    IS_CHAIN = True  # ClassifierChain flag
    IS_ORDER_NONSCORED = True
    OUTDIR = f"{OUTDIR}/chain_nonscored"

params = {
    "num_leaves": p_py.num_leaves,
    "max_depth": p_py.max_depth,
    "min_child_samples": p_py.min_child_samples,
    "objective": "binary",
    "learning_rate": LR,
    # "n_estimators": N_ESTIMATORS,
}
SEEDS = p_py.SEEDS  # 乱数シード指定してるが、MultilabelStratifiedKFoldは shuffle=False にしている
print(params)
print(SEEDS)
...
def mapping_and_filter(train, train_targets, test, train_targets_nonscored):
    ...
    print(
        "train_targets.shape, train_targets_nonscored.shape:",
        train_targets.shape,
